...
from src.institutional.presentation.commercial_refrigeration import DEVELOPMENT_VIDEO_URL, FAQS


class CommercialRefrigerationTests(SimpleTestCase):
    def test_page_metadata_visible_faq_and_integrations(self):
        response = self.client.get(reverse("institutional:refrigeracao_comercial") + "?utm_source=test")
        self.assertEqual(response.status_code, 200)
        html = response.content.decode()
        self.assertEqual(len(re.findall(r"<h1\b", html)), 1)
        self.assertContains(
            response,
            "Refrigeração Comercial e Câmaras Frigoríficas em São Paulo | Smart Control Brasil",
        )
        self.assertContains(
            response,
            'rel="canonical" href="https://www.smartcontrolbrasil.com.br/refrigeracao-comercial/"',
        )
        graph = json.loads(
        ...
        self.assertNotContains(response, "conserto de geladeira")
        self.assertNotContains(response, "representante oficial exclusivo")

    @override_settings(DEBUG=False, COMMERCIAL_REFRIGERATION_VIDEO_URL="")
    def test_production_keeps_popup_without_template_video(self):
        response = self.client.get("/refrigeracao-comercial/")
        self.assertContains(response, "data-cr-video")
    ...
    @override_settings(DEBUG=True, COMMERCIAL_REFRIGERATION_VIDEO_URL="")
    ...
    @override_settings(DEBUG=False, COMMERCIAL_REFRIGERATION_VIDEO_URL="https://youtu.be/abcdefghijk")
    def test_configured_youtube_url_loads_only_on_interaction(self):
        response = self.client.get("/refrigeracao-comercial/")
        self.assertContains(
            response,
            'data-embed-url="https://www.youtube-nocookie.com/embed/abcdefghijk?autoplay=1"',
        )
        self.assertNotContains(response, "<iframe")

    @override_settings(DEBUG=False, COMMERCIAL_REFRIGERATION_VIDEO_URL="https://untrusted.example/watch?v=DZLlw5BNQ3g")
    ...
    @override_settings(DEBUG=False, COMMERCIAL_REFRIGERATION_VIDEO_URL=DEVELOPMENT_VIDEO_URL)
