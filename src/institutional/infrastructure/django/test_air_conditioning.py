...
class AirConditioningTests(SimpleTestCase):
    def test_page_metadata_visible_faq_and_integrations(self):
        response = self.client.get(reverse("institutional:ar_condicionado") + "?utm_source=test")
        self.assertEqual(response.status_code, 200)
        html = response.content.decode()
        self.assertEqual(len(re.findall(r"<h1\b", html)), 1)
        self.assertContains(
            response,
            "Ar-Condicionado em São Paulo | Instalação e Manutenção | Smart Control Brasil",
        )
        self.assertContains(
            response,
            'rel="canonical" href="https://www.smartcontrolbrasil.com.br/ar-condicionado/"',
        )
        graph = json.loads(
    ...
    @override_settings(DEBUG=False, AIR_CONDITIONING_VIDEO_URL="")
    def test_production_keeps_popup_without_template_video(self):
        response = self.client.get("/ar-condicionado/")
        self.assertContains(response, "data-ac-video")
    ...
    @override_settings(DEBUG=True, AIR_CONDITIONING_VIDEO_URL="")
    ...
    @override_settings(DEBUG=False, AIR_CONDITIONING_VIDEO_URL="https://youtu.be/abcdefghijk")
    ...
    @override_settings(DEBUG=False, AIR_CONDITIONING_VIDEO_URL="https://untrusted.example/watch?v=DZLlw5BNQ3g")
    ...
    @override_settings(DEBUG=False, AIR_CONDITIONING_VIDEO_URL=DEVELOPMENT_VIDEO_URL)
